...
def initial_step_size(fun, t0, y0, order, rtol, atol, f0):
    ...
    h1 = jnp.where(
        ...
        (0.01 / jnp.max(d1 + d2)) ** (1.0 / (order + 1.0)),
    )
...
def optimal_step_size(
    last_step, mean_error_ratio, safety=0.9, ifactor=10.0, dfactor=0.2, order=5.0, max_step=1.0
):
    """Compute optimal Runge-Kutta stepsize."""
    dfactor = jnp.where(mean_error_ratio < 1, 1.0, dfactor)

    factor = jnp.minimum(ifactor, jnp.maximum(mean_error_ratio ** (-1.0 / order) * safety, dfactor))
    step_size = jnp.where(mean_error_ratio == 0, last_step * ifactor, last_step * factor)
    return jnp.where(step_size > max_step, max_step, step_size)
...
def _odeint(func, rtol, atol, mxstep, y0, ts, *args):
    ...
    def scan_fun(carry, target_t):
        ...
        def body_fun(state):
            ...
            return map(partial(jnp.where, error_ratio <= 1.0), new, old)
...
def _odeint_rev(func, rtol, atol, mxstep, res, g):
    ...
    init_carry = (g[-1], 0.0, tree_map(jnp.zeros_like, args))
